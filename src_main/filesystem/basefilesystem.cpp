...
void CBaseFileSystem::HandleOpenRegularFile( CFileOpenInfo &openInfo, bool bIsAbsolutePath )
{
	...
	if ( fp )
	{
		...
		if ( m_bOutputDebugString )
		{
			...
			__android_log_print(ANDROID_LOG_INFO, "CBaseFileSystem", "fs_debug: %s", openInfo.m_AbsolutePath);
#elif defined(_LINUX)
...
#elif defined(__ANDROID__)
		__android_log_print(ANDROID_LOG_WARN, "CBaseFileSystem", "%s", warningtext);
#endif
	}
}
...
{
	size_t bufSizeInWchars = bufSizeInBytes / sizeof(wchar_t);
	...
	if ( dwResult )
	{
		wcsncpy(buf, info.szTypeName, bufSizeInWchars);
		buf[bufSizeInWchars - 1] = L'\0';
		return true;
	}
	else
#endif
...
#ifdef __ANDROID__
		_snwprintf(buf, bufSizeInWchars - 1, L".%s", ext); // fake ANSI-only swprintf
#else
		_snwprintf(buf, bufSizeInWchars - 1, L".%S", ext);
#endif
		buf[bufSizeInWchars - 1] = L'\0';
	}
	return false;
