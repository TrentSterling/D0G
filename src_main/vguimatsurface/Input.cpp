...
//-----------------------------------------------------------------------------
#ifdef __ANDROID__
static ANDR_INPUTHANDLER s_ChainedInputHandler;
#else
		...
		case AKEY_EVENT_ACTION_MULTIPLE:
			{
				...
				event.m_nType = IE_KeyCodeTyped;
				event.m_nData = KeyCode_VirtualKeyToVGUI(AKeyEvent_getKeyCode(aEvent));
				int32_t i = AKeyEvent_getRepeatCount(aEvent);
				while (i-- > 0)
					g_pInputSystem->PostUserEvent(event);
				return retVal;
			}
...
{
#ifdef __ANDROID__
	struct android_app *app = (struct android_app *)hwnd;
	s_ChainedInputHandler = app->onInputEvent;
	app->onInputEvent = MatSurfaceInputHandler;
#else
...
void InputDetachFromWindow(void *hwnd)
{
	...
	if (s_ChainedInputHandler)
	{
		((struct android_app *)hwnd)->onInputEvent = s_ChainedInputHandler;
		s_ChainedInputHandler = NULL;
	}
#else
	if ( s_ChainedWindowProc )
	{
