...
#ifdef __ANDROID__
	FontManager().SetLanguage(ANDR_GetLanguageString());
#else
	char language[64];
	bool bValid;
	if ( IsPC() )
...
bool CMatSystemSurface::AddCustomFontFile( const char *fontFileName )
{
	...
	{
		bFound = (IsX360() && (g_pFullFileSystem->GetDVDMode() == DVDMODE_STRICT)) || g_pFullFileSystem->FileExists(fontFileName, "GAME");
	...
	bool success = FontManager().AddFontFile(fullPath);
	if (!success)
...
#elif !defined(_X360) && !defined(__ANDROID__)
