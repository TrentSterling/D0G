...
#else
		CThreadEvent &waitCallFromMaster = GetCallHandle();
		CThreadEvent &waitSharedQueue = m_SharedQueue.GetEventHandle();
		CThreadEvent &waitDirectQueue = m_DirectQueue.GetEventHandle();
#endif

		m_pOwner->m_nIdleThreads++;
		m_IdleEvent.Set();
#ifdef _WIN32
		while (!bExit && (Wait(ARRAYSIZE(waitHandles), waitHandles) != WAIT_FAILED))
#else
		while (!bExit)
#endif
		{
			...
			for (;;)
			{
				if (waitSharedQueue.Wait(100))
					break;
				if (waitDirectQueue.Wait(10))
					break;
				if (waitCallFromMaster.Wait(0))
					break;
			}
#endif
			if ( PeekCall() )
...
int CThreadPool::YieldWait(CThreadEvent **pEvents, int nEvents, bool bWaitAll, unsigned timeout)
{
	...
	Assert(bWaitAll && (nEvents == 1) && (timeout == TT_INFINITE));
	if (pEvents[0]->Wait(0))
		return WAIT_OBJECT_0;
	CJob *pJob;
	unsigned waitTimeout = 0;
	for (;;)
	{
		if (m_SharedQueue.Pop(&pJob))
		{
			ServiceJobAndRelease(pJob);
			m_nJobs--;
			if (!(pEvents[0]->Wait(waitTimeout)))
				return WAIT_OBJECT_0;
		}
		else
		{
			waitTimeout = TT_INFINITE;
			if (pEvents[0]->Wait(TT_INFINITE))
				return WAIT_OBJECT_0;
		}
	}
	return WAIT_OBJECT_0;
...
#else
					ThreadSetAffinity( (ThreadHandle_t)m_Threads[i]->GetThreadId(), 1 << iProc );
#endif
				}
			}
			else
...
#else
					ThreadSetAffinity( (ThreadHandle_t)m_Threads[i]->GetThreadId(), pAffinityTable[i] );
#endif
				}
			}
		}
