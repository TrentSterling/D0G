...
TEMPLATE_FUNCTION_TABLE( void, R_LightEffectsWorldFunctionTable, ( const LightDesc_t* pLightDesc, const lightpos_t *light, const Vector& normal, Vector &dest ), 256 )
{
	...
	if( LightType1 != (int)MATERIAL_LIGHT_DISABLE )
	{
	...
	if( LightType2 != (int)MATERIAL_LIGHT_DISABLE )
	{
	...
	if( LightType3 != (int)MATERIAL_LIGHT_DISABLE )
	{
	...
	if( LightType4 != (int)MATERIAL_LIGHT_DISABLE )
	{
