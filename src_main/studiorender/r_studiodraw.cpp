				...
				float r_blend, IMaterial* pMaterial, int lod, ColorMeshInfo_t *pColorMeshes  )
{
	...
	// On Android, using DX8 models because DX9 models are useless if the device has <256 vertex uniforms.
	bool bUseSOFlex = !IsAndroid() && g_pMaterialSystemHardwareConfig->SupportsStreamOffset() && !bUseHWFlex;
