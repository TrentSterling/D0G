...
void CSmallBlockPool::Init( unsigned nBlockSize, byte *pBase, unsigned initialCommit )
{
	...
	if ( initialCommit )
	{
		initialCommit = MemAlign( initialCommit, MEMSTD_PAGE_SIZE );
	...
	if ( !pResult )
	{
		...
		for (;;)
		{
			...
			else
			{
				...
				{
					if ( pCommitLimit + MEMSTD_COMMIT_SIZE <= m_pAllocLimit )
					{
						if ( !VirtualAlloc( pCommitLimit, MEMSTD_COMMIT_SIZE, VA_COMMIT_FLAGS, PAGE_READWRITE ) )
						{
							Assert( 0 );
							return NULL;
						}

						m_pCommitLimit = pCommitLimit + MEMSTD_COMMIT_SIZE;
					}
					else
					{
						return NULL;
...
int CSmallBlockPool::Compact()
{
	...
	if ( m_FreeList.Count() )
	{
		...
		{
			byte *pNewCommitLimit = MemAlign( (byte *)m_pNextAlloc, MEMSTD_PAGE_SIZE );
