...
extern const u32x4 g_SIMD_clear_wmask_NEON;
extern const u32x4 g_SIMD_ComponentMask_NEON[4];
extern const i32x4 g_SIMD_signshift_NEON;
#endif

#define SIMD_UNOP(x, op) FORCEINLINE fltx4 x(const fltx4 &a) {return op(a);}
#define SIMD_BINOP(x, op) FORCEINLINE fltx4 x(const fltx4 &a, const fltx4 &b) {return op(a, b);}
...
{
	return ((SubInt(a, 0) & 0x80000000) >> 31) | // sign(x) -> bit 0
	       ((SubInt(a, 1) & 0x80000000) >> 30) | // sign(y) -> bit 1
	       ((SubInt(a, 2) & 0x80000000) >> 29) | // sign(z) -> bit 2
	       ((SubInt(a, 3) & 0x80000000) >> 28);  // sign(w) -> bit 3
}
...
SIMD_BINOP(AddSIMD, __vaddfp) // a + b

SIMD_BINOP(SubSIMD, __vsubfp) // a - b

SIMD_BINOP(MulSIMD, __vmulfp) // a * b
...
SIMD_BINOP(Dot3SIMD, __vmsum3fp)

SIMD_BINOP(Dot4SIMD, __vmsum4fp)
...
SIMD_UNOP(ArcSinSIMD, XMVectorASin)


// DivSIMD defined further down, since it uses ReciprocalSIMD

SIMD_BINOP(MaxSIMD, __vmaxfp) // max(a, b)

SIMD_BINOP(MinSIMD, __vminfp) // min(a, b)

SIMD_BINOP(AndSIMD, __vand) // a & b
...
SIMD_BINOP(XorSIMD, __vxor) // a ^ b

SIMD_BINOP(OrSIMD, __vor) // a | b

SIMD_UNOP(NegSIMD, XMVectorNegate) // megate: -a
...
SIMD_BINOP(CmpEqSIMD, __vcmpeqfp) // (a == b) ? ~0 : 0

SIMD_BINOP(CmpGtSIMD, __vcmpgtfp) // (a > b) ? ~0 : 0

SIMD_BINOP(CmpGeSIMD, __vcmpgefp) // (a >= b) ? ~0 : 0
...
SIMD_BINOP(CmpInBoundsSIMD, XMVectorInBounds) // (a <= b && a >= -b) ? ~0 : 0
...
SIMD_UNOP(CeilSIMD, __vrfip) // Round towards positive infinity

SIMD_UNOP(RoundSIMD, __vrfin) // Round towards nearest integer

SIMD_UNOP(FloorSIMD, __vrfim) // Round towards negative infinity

SIMD_UNOP(SqrtEstSIMD, XMVectorSqrtEst) // sqrt(a), more or less

SIMD_UNOP(SqrtSIMD, XMVectorSqrt) // sqrt(a)

SIMD_UNOP(ReciprocalSqrtEstSIMD, __vrsqrtefp) // 1/sqrt(a), more or less

SIMD_UNOP(ReciprocalSqrtSIMD, XMVectorReciprocalSqrt) // 1/sqrt(a)

SIMD_UNOP(ReciprocalEstSIMD, __vrefp) // 1/a, more or less

/// 1/x for all 4 values. uses reciprocal approximation instruction plus newton iteration.
/// No error checking!
SIMD_UNOP(ReciprocalSIMD, XMVectorReciprocal) // 1/a
...
FORCEINLINE fltx4 LoadAlignedSIMD(const float *pSIMD)
{
	return *((const fltx4 *)pSIMD);
}

FORCEINLINE fltx4 AndSIMD(const fltx4 &a, const fltx4 &b) // a & b
{
	u32x4 retVal = vandq_u32(*((u32x4 *)(&a)), *((u32x4 *)(&b)));
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 AndNotSIMD(const fltx4 &a, const fltx4 &b) // a & ~b
{
	u32x4 retVal = vbicq_u32(*((u32x4 *)(&a)), *((u32x4 *)(&b)));
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 XorSIMD(const fltx4 &a, const fltx4 &b) // a ^ b
{
	u32x4 retVal = veorq_u32(*((u32x4 *)(&a)), *((u32x4 *)(&b)));
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 OrSIMD(const fltx4 &a, const fltx4 &b) // a | b
{
	u32x4 retVal = vorrq_u32(*((u32x4 *)(&a)), *((u32x4 *)(&b)));
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 SetWToZeroSIMD(const fltx4 &a)
{
	return AndSIMD(a, *((fltx4 *)(&g_SIMD_clear_wmask_NEON)));
}
...
#define LoadUnalignedSIMD LoadAlignedSIMD
#define LoadUnaligned3SIMD LoadAlignedSIMD

FORCEINLINE fltx4 ReplicateIX4(int nValue)
{
	return vdupq_n_f32(*((float *)(&nValue)));
}
...
FORCEINLINE fltx4 MaskedAssign(const fltx4 &mask, const fltx4 &newValue, const fltx4 &oldValue)
{
	return OrSIMD(AndSIMD(mask, newValue), AndNotSIMD(mask, oldValue));
}

FORCEINLINE fltx4 SplatXSIMD(const fltx4 &a)
{
	return ReplicateX4(SubFloat(a, 0));
}

FORCEINLINE fltx4 SplatYSIMD(const fltx4 &a)
{
	return ReplicateX4(SubFloat(a, 1));
}

FORCEINLINE fltx4 SplatZSIMD(const fltx4 &a)
{
	return ReplicateX4(SubFloat(a, 2));
}

FORCEINLINE fltx4 SplatWSIMD(const fltx4 &a)
{
	return ReplicateX4(SubFloat(a, 3));
}

FORCEINLINE fltx4 SetXSIMD(const fltx4 &a, const fltx4 &x)
{
	return MaskedAssign(*((fltx4 *)g_SIMD_ComponentMask_NEON), x, a);
}

FORCEINLINE fltx4 SetYSIMD(const fltx4 &a, const fltx4 &y)
{
	return MaskedAssign(*((fltx4 *)(g_SIMD_ComponentMask_NEON + 1)), y, a);
}

FORCEINLINE fltx4 SetZSIMD(const fltx4 &a, const fltx4 &z)
{
	return MaskedAssign(*((fltx4 *)(g_SIMD_ComponentMask_NEON + 2)), z, a);
}

FORCEINLINE fltx4 SetWSIMD(const fltx4 &a, const fltx4 &w)
{
	return MaskedAssign(*((fltx4 *)(g_SIMD_ComponentMask_NEON + 3)), w, a);
}

FORCEINLINE fltx4 SetComponentSIMD(const fltx4 &a, int nComponent, float flValue)
{
	return MaskedAssign(*((fltx4 *)(g_SIMD_ComponentMask_NEON + nComponent)), ReplicateX4(flValue), a);
}

FORCEINLINE fltx4 RotateLeft(const fltx4 &a) // abcd -> bcda
{
	return vextq_f32(a, a, 1);
}

FORCEINLINE fltx4 RotateLeft2(const fltx4 &a) // abcd -> cdab
{
	return vextq_f32(a, a, 2);
}

FORCEINLINE fltx4 RotateRight(const fltx4 &a) // abcd -> dabc
{
	return vextq_f32(a, a, 3);
}

#define RotateRight2 RotateLeft2

SIMD_BINOP(AddSIMD, vaddq_f32) // a + b
SIMD_BINOP(SubSIMD, vsubq_f32) // a - b
SIMD_BINOP(MulSIMD, vmulq_f32) // a * b
...
// SISD version is significantly faster
FORCEINLINE fltx4 Dot4SIMD(const fltx4 &a, const fltx4 &b)
{
	fltx4 m = MulSIMD(a, b);
	return ReplicateX4(SubFloat(m, 0) + SubFloat(m, 1) + SubFloat(m, 2) + SubFloat(m, 3));
}

FORCEINLINE fltx4 Dot3SIMD(const fltx4 &a, const fltx4 &b)
{
	fltx4 m = MulSIMD(a, b);
	return ReplicateX4(SubFloat(m, 0) + SubFloat(m, 1) + SubFloat(m, 2));
}
...
SIMD_UNOP(NegSIMD, vnegq_f32) // negate: -a

FORCEINLINE int TestSignSIMD(const fltx4 &a) // mask of which floats have the high bit set
{
	// Also tried vpadd, but it's slower - wastes 75% of the optimization
	u32x4 retVal = vshlq_u32(vandq_u32(*((u32x4 *)(&a)), g_SIMD_signmask_NEON), g_SIMD_signshift_NEON);
	u32x4 m = vextq_u32(retVal, retVal, 1);
	retVal = vorrq_u32(retVal, m);
	m = vextq_u32(m, m, 1);
	retVal = vorrq_u32(retVal, m);
	m = vextq_u32(m, m, 1);
	retVal = vorrq_u32(retVal, m);
...
FORCEINLINE bool IsAnyNegative(const fltx4 &a) // (a.x < 0) || (a.y < 0) || (a.z < 0) || (a.w < 0)
{
	return TestSignSIMD(a) != 0;
}

FORCEINLINE fltx4 CmpEqSIMD(const fltx4 &a, const fltx4 &b) // (a == b) ? ~0 : 0
{
	u32x4 retVal = vceqq_f32(a, b);
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 CmpGtSIMD(const fltx4 &a, const fltx4 &b) // (a > b) ? ~0 : 0
{
	u32x4 retVal = vcgtq_f32(a, b);
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 CmpGeSIMD(const fltx4 &a, const fltx4 &b) // (a >= b) ? ~0 : 0
{
	u32x4 retVal = vcgeq_f32(a, b);
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 CmpLtSIMD(const fltx4 &a, const fltx4 &b) // (a < b) ? ~0 : 0
{
	u32x4 retVal = vcltq_f32(a, b);
	return *((fltx4 *)(&retVal));
}

FORCEINLINE fltx4 CmpLeSIMD(const fltx4 &a, const fltx4 &b) // (a <= b) ? ~0 : 0
{
	u32x4 retVal = vcleq_f32(a, b);
	return *((fltx4 *)(&retVal));
}

FORCEINLINE bool IsAllGreaterThan(const fltx4 &a, const fltx4 &b) // for branching when a.xyzw > b.xyzw
{
	return TestSignSIMD(CmpLeSIMD(a, b)) == 0;
}

FORCEINLINE bool IsAllGreaterThanOrEq(const fltx4 &a, const fltx4 &b) // for branching when a.xyzw >= b.xyzw
{
	return TestSignSIMD(CmpLtSIMD(a, b)) == 0;
}

FORCEINLINE bool IsAllEqual(const fltx4 &a, const fltx4 &b) // for branching when a.xyzw == b.xyzw
{
...
SIMD_BINOP(MinSIMD, vminq_f32) // min(a, b)
SIMD_BINOP(MaxSIMD, vmaxq_f32) // max(a, b)

// NEON rounds towards zero when converting to integer,
// so rounding positive numbers outputs a less number,
// while rounding negative ones outputs a greater one.
// So if the number is less, add 1 to the result. For
// flooring, it's the opposite: subtract 1 if greater.
FORCEINLINE fltx4 CeilSIMD(const fltx4 &a)
{
...
{
	return TestSignSIMD(CmpEqSIMD(a, Four_Zeros)) == 0xf;
}

SIMD_UNOP(ReciprocalSqrtEstSIMD, vrsqrteq_f32) // 1/sqrt(a), more or less

// Two iterations in these functions are important,
// otherwise simple integer results will be incorrect.
// They don't affect performance very much - the NEON
// sqrt is twice as fast as its SISD cmath version.
FORCEINLINE fltx4 ReciprocalSqrtSIMD(const fltx4 &a) // 1/sqrt(a)
{
	fltx4 guess = ReciprocalSqrtEstSIMD(a);
	// newton iteration for 1/sqrt(a) : y(n+1) = 1/2 (y(n)*(3-a*y(n)^2));
	guess = MulSIMD(guess, vrsqrtsq_f32(a, MulSIMD(guess, guess)));
	return MulSIMD(guess, vrsqrtsq_f32(a, MulSIMD(guess, guess)));
}

SIMD_UNOP(ReciprocalEstSIMD, vrecpeq_f32) // 1/a, more or less
...
FORCEINLINE fltx4 ReciprocalSIMD(const fltx4 &a) // 1/a
{
	...
	return SubSIMD(AddSIMD(ret, ret), MulSIMD(a, MulSIMD(ret, ret)));
}

FORCEINLINE fltx4 SqrtEstSIMD(const fltx4 &a) // sqrt(a), more or less
{
	return ReciprocalEstSIMD(ReciprocalSqrtEstSIMD(a));
}

FORCEINLINE fltx4 SqrtSIMD(const fltx4 &a) // sqrt(a)
{
	return AndNotSIMD(ReciprocalSIMD(ReciprocalSqrtSIMD(a)), CmpEqSIMD(a, Four_Zeros));
}

// Slow - read X360 comment
FORCEINLINE fltx4 DivSIMD(const fltx4 &a, const fltx4 &b) // a / b
{
	return MulSIMD(ReciprocalSIMD(b), a);
}
...
FORCEINLINE fltx4 ReciprocalSaturateSIMD(const fltx4 & a)
{
	return ReciprocalSIMD(OrSIMD(a, AndSIMD(Four_Epsilons, CmpEqSIMD(a, Four_Zeros))));
}
...
FORCEINLINE fltx4 ClampVectorSIMD(const fltx4 &in, const fltx4 &min, const fltx4 &max)
{
	return MaxSIMD(min, MinSIMD(max, in));
}
...
FORCEINLINE fltx4 FindLowestSIMD3(const fltx4 &a)
{
	return SplatXSIMD(MinSIMD(MinSIMD(a, RotateLeft(a)), RotateLeft2(a)));
}

FORCEINLINE fltx4 FindHighestSIMD3(const fltx4 &a)
{
	return SplatXSIMD(MaxSIMD(MaxSIMD(a, RotateLeft(a)), RotateLeft2(a)));
}

FORCEINLINE i32x4 LoadAlignedIntSIMD(const int32 * RESTRICT pSIMD)
{
	return *((const i32x4 *)pSIMD);
}

#define LoadUnalignedIntSIMD LoadAlignedIntSIMD
...
FORCEINLINE void StoreAlignedIntSIMD(intx4 &pSIMD, const fltx4 &a)
{
	*((i32x4 *)pSIMD.Base()) = *((i32x4 *)(&a));
}

#define StoreUnalignedIntSIMD StoreAlignedIntSIMD

FORCEINLINE fltx4 UnsignedIntConvertToFltSIMD(const u32x4 &a)
{
	return vcvtq_f32_u32(a);
}

FORCEINLINE fltx4 SignedIntConvertToFltSIMD(const i32x4 &a)
{
	return vcvtq_f32_s32(a);
}

// Isn't use anywhere, but a caution: NEON treats negative shift as
// right shift. Because negative shift is not an expected use of the
// function, this case is not handled.
FORCEINLINE i32x4 IntShiftLeftWordSIMD(const i32x4 &a, const i32x4 &b)
{
	u32x4 retVal = vshlq_u32(*((u32x4 *)(&a)), b);
	return *((i32x4 *)(&retVal));
}

FORCEINLINE void ConvertStoreAsIntsSIMD(intx4 * RESTRICT pDest, const fltx4 &vSrc)
{
	*((i32x4 *)(pDest->Base())) = vcvtq_s32_f32(vSrc);
}
...
SIMD_BINOP(AndSIMD, _mm_and_ps) // a & b

SIMD_BINOP(AndNotSIMD, _mm_andnot_ps) // a & ~b

SIMD_BINOP(XorSIMD, _mm_xor_ps) // a ^ b

SIMD_BINOP(OrSIMD, _mm_or_ps) // a | b
...
SIMD_BINOP(AddSIMD, _mm_add_ps) // a + b
SIMD_BINOP(SubSIMD, _mm_sub_ps) // a - b
SIMD_BINOP(MulSIMD, _mm_mul_ps) // a * b
SIMD_BINOP(DivSIMD, _mm_div_ps) // a / b
...
SIMD_BINOP(CmpEqSIMD, _mm_cmpeq_ps) // (a == b) ? ~0 : 0
SIMD_BINOP(CmpGtSIMD, _mm_cmpgt_ps) // (a > b) ? ~0 : 0
SIMD_BINOP(CmpGeSIMD, _mm_cmpge_ps) // (a >= b) ? ~0 : 0
SIMD_BINOP(CmpLtSIMD, _mm_cmplt_ps) // (a < b) ? ~0 : 0
SIMD_BINOP(CmpLeSIMD, _mm_cmple_ps) // (a <= b) ? ~0 : 0
...
SIMD_BINOP(MinSIMD, _mm_min_ps) // min(a, b)
SIMD_BINOP(MaxSIMD, _mm_max_ps) // max(a, b)
...
SIMD_UNOP(SqrtEstSIMD, _mm_sqrt_ps) // sqrt(a), more or less
SIMD_UNOP(SqrtSIMD, _mm_sqrt_ps) // sqrt(a)

SIMD_UNOP(ReciprocalSqrtEstSIMD, _mm_rsqrt_ps) // 1/sqrt(a), more or less
...
SIMD_UNOP(ReciprocalEstSIMD, _mm_rcp_ps) // 1/a, more or less
