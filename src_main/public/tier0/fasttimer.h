...
#if defined(__ANDROID__)
		// Android uses variable CPU frequency, but timespec uses nanoseconds,
		// so assume 1GHz.
		g_ClockSpeed = 1000000000;
		g_dwClockSpeed = 1000000000;

		g_ClockSpeedMicrosecondsMultiplier = 0.001;
		g_ClockSpeedMillisecondsMultiplier = 0.000001;
		g_ClockSpeedSecondsMultiplier = 0.000000001;
#else
#if defined( _X360 ) && !defined( _CERT )
		PMCStart();
...
#if defined(__ANDROID__)
	struct timespec res;
	clock_gettime(CLOCK_MONOTONIC_HR, &res);
	m_Int64 = res.tv_sec * 1000000000LL + res.tv_nsec;
#elif defined( _X360 )
#if !defined( _CERT )
