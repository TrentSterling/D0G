...
void FileOpenDialog::Init( const char *title, KeyValues *pContextKeyValues )
{
...
#ifdef __ANDROID__
	m_pOpenInExplorerButton->SetVisible(false);
#endif
	...
	char pLocalPath[255];
	g_pFullFileSystem->GetCurrentDirectory(pLocalPath, sizeof(pLocalPath));
#if defined(_LINUX) && !defined(__ANDROID__)
	if (!pLocalPath[0])
	{
		const char *pszHomeDir = getenv("HOME");
		Q_strncpy(pLocalPath, pszHomeDir, sizeof(pLocalPath));
	}
#endif
...
//-----------------------------------------------------------------------------
static char *StripWhitespaceWorker(int cchLength, char *pch, bool *pbStrippedWhitespace)
{
	// walk backwards from the end of the string, killing any whitespace
	*pbStrippedWhitespace = false;

	char *pchEnd = pch + cchLength;
	while (--pchEnd >= pch)
	{
		if (!isspace(*pchEnd))
			break;
...
static bool StripPrecedingAndTrailingWhitespace(char *pch)
{
	// duplicate on stack
	int cch = Q_strlen( pch );
	int cubDest = cch + 1;
	char *pchT = (char *)stackalloc(cubDest);
	Q_strncpy(pchT, pch, cubDest);

	bool bStrippedWhitespace = false;
	pchT = StripWhitespaceWorker(cch, pch, &bStrippedWhitespace);

	// copy back, if necessary
	if (bStrippedWhitespace)
		Q_strncpy(pch, pchT, cubDest);
...
void FileOpenDialog::ValidatePath()
{
	...
	StripPrecedingAndTrailingWhitespace(fullpath);
