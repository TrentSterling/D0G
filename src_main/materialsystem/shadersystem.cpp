...
void CShaderSystem::LoadAllShaderDLLs( )
{
	...
	// Load up standard shader DLLs...
#ifdef __ANDROID__
	// Contains both OES2 and OES3 shaders since the difference between the two ShaderAPIs is little.
	LoadShaderDLL("stdshader_oes2");
#else
	int dxSupportLevel = HardwareConfig()->GetMaxDXSupportLevel();
	Assert( dxSupportLevel >= 60 );
	dxSupportLevel /= 10;
...
void CShaderSystem::LoadModShaderDLLs( int dxSupportLevel )
{
...
#ifdef __ANDROID__
	LoadShaderDLL("game_shader_oes2", pModShaderPathID, true);
#else
	int dxStart = 6;
	...
	const char *pFilename = g_pFullFileSystem->FindFirstEx(
		IsAndroid() ? "game_shader_oes2_generic*" : "game_shader_generic*", pModShaderPathID, &findHandle);
	while ( pFilename )
	{
