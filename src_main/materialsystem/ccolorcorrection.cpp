...
void ColorCorrectionLookup_t::AllocTexture()
{
	...
	int size;
#ifdef __ANDROID__
	int maxSize = HardwareConfig()->MaxTextureDepth();
	if (maxSize >= COLOR_CORRECTION_TEXTURE_SIZE)
		size = COLOR_CORRECTION_TEXTURE_SIZE;
	else if (maxSize >= COLOR_CORRECTION_SMALL_TEXTURE_SIZE)
		size = COLOR_CORRECTION_SMALL_TEXTURE_SIZE;
	else
		size = 1;
#else
	size = COLOR_CORRECTION_TEXTURE_SIZE;
	...
	m_pColorCorrectionTexture = ITextureInternal::CreateProceduralTexture( name, TEXTURE_GROUP_OTHER,
		size, size, size, IMAGE_FORMAT_BGRX8888,
	...
	Warning("ColorCorrectionLookup_t::AllocTexture: size = %d\n", size);
