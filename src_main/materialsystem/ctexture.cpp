...
{
#ifdef __ANDROID__
	return g_pShaderAPI->GetNearestSupportedFormat(srcFormat);
#endif
...
int CTexture::ComputeActualMipCount( ) const
{
	...
	if ( !IsAndroid() && !( m_nFlags & TEXTUREFLAGS_ALL_MIPS ) )
	{
		int nNumMipLevels = 1;
...
int CTexture::ComputeActualSize( bool bIgnorePicmip, IVTFTexture *pVTFTexture )
{
	...
	//
	int iHwWidth, iHwHeight, iHwDepth = 1;
#ifdef __ANDROID__
	if (nClampZ > 1)
		iHwWidth = iHwHeight = iHwDepth = HardwareConfig()->MaxTextureDepth();
	else if (IsCubeMap())
		iHwWidth = iHwHeight = HardwareConfig()->MaxCubeMapTextureSize();
	else if (IsRenderTarget())
		iHwWidth = iHwHeight = HardwareConfig()->MaxRenderTargetSize();
	else
#endif
	{
