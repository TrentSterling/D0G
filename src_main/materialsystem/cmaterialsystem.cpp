...
void CMaterialSystem::SetShaderAPI( char const *pShaderAPIDLL )
{
	...
	if ( !pShaderAPIDLL )
	{
		pShaderAPIDLL = IsAndroid() ? "shaderapioes2" : "shaderapidx9";
	}
...
#endif
	pConfig->m_bShadowDepthTexture = !IsAndroid() && r_flashlightdepthtexture.GetBool();
...
bool CMaterialSystem::OverrideConfig( const MaterialSystem_Config_t &_config, bool forceUpdate )
{
	...
	if ( (IsX360() || IsAndroid() || !config.Windowed()) && (config.WaitForVSync() != g_config.WaitForVSync()) )
	{
