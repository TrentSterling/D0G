...
public:
	...
	virtual void RegenerateTextureBits( ITexture *pTexture, IVTFTexture *pVTFTexture, Rect_t *pSubRect )
	{
		...
		int nDepth = pVTFTexture->Depth();
#ifdef __ANDROID__
		Assert((nWidth == nHeight) && (nWidth == nDepth));
		Assert((nWidth == COLOR_CORRECTION_TEXTURE_SIZE) || (nWidth == COLOR_CORRECTION_SMALL_TEXTURE_SIZE) || (nWidth == 1));
#else
		...
		for ( int z = 0; z < nDepth; ++z )
		{
			...
			for ( int y = 0; y < nHeight; ++y )
			{
				...
				for (int x = 0; x < nWidth; ++x)
				{
					...
					color24 col;
					int r = 0, g = 0, b = 0;
					if (nWidth == COLOR_CORRECTION_SMALL_TEXTURE_SIZE)
					{
						x <<= 1; y <<= 1; z <<= 1;
						for (int l = 8; l--; )
						{
						...
						x >>= 1; y >>= 1; z >>= 1;
						pixelWriter.WritePixel(col.r >> 3, col.g >> 3, col.b >> 3, 255);
					}
					else
#endif
					{
